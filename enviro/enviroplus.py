...
def send_data(variable, data, unit):
    ...
    measurements = [
            {
                ...
                'fields': {
                    'deviceId': '0',
                    'sensor': unicode(variable),
                    'data': data,
                    'unit': unicode(unit),
                    'ts': unicode(timestamp)
                }
            }
        ]
    try:
        res = influx_client.write_points(measurements)

        #doc_ref = db.collection(unicode(variable)).document(str(uuid.uuid4()))
        #doc_ref.set(doc)
    except Exception as ex:
        logging.info(ex)
        errmessage = "data not sent {}: {:.1f} {}".format(variable[:4], data, unit)
...
def display_text(variable, data, unit):
    ...
    st7735.display(img.rotate(180))
    #send_data(variable, data, unit)
